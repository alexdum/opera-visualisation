...
7. **RATE/ACRR Nodata Fidelity**: Preserve each product's own nodata cells as transparent. Do not fill or mask RATE/ACRR from DBZH coverage, geographic masks, or morphology; DBZH availability does not prove that a derived precipitation product is valid. Correct only renderer-created artifacts by classifying finite nodata sentinels and NaN undetect cells before reprojection. Pixel analysis and map rendering must retain the product-specific distinction between undetect and nodata.
