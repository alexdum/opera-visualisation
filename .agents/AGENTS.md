...
5. **Native Resolution Rendering and Backend Parity**: COG and GeoZarr MUST classify undetect/nodata before reprojection and use the same Web Mercator destination-grid sizing, bounded by the cropped native source width and `max_size`. Do not artificially upsample either backend on the server; WebGL may enlarge the shared native-resolution texture for display. A catalog frame created from the same source grid MUST produce identical packed measurement and quality bytes from COG and GeoZarr for the same bounds and rendering options.
6. **Pixel-Centre Crop Parity**: Select COG crop windows from native pixel-centre coordinates using the same inclusive bounds and one-cell edge padding as GeoZarr. Do not derive COG windows independently from fractional raster-edge rounding; that can create one-cell differences, especially for the native 2 km RATE and ACRR grids even when the 1 km DBZH grid appears aligned. Regression tests MUST cover both native resolutions.
7. **RATE/ACRR Nodata Fidelity**: Preserve each product's own nodata cells as transparent. Do not fill or mask RATE/ACRR from DBZH coverage, geographic masks, or morphology; DBZH availability does not prove that a derived precipitation product is valid. Correct only renderer-created artifacts by classifying finite nodata sentinels and NaN undetect cells before reprojection. Pixel analysis and map rendering must retain the product-specific distinction between undetect and nodata. Radial or sector-shaped RATE/ACRR gaps, including those around Ireland and Scotland, may already exist as nodata in the upstream NIMBUS Lambert grid. Web Mercator reprojection may stretch or slightly resample their edges but does not create the sectors. Verify the source HDF5/COG status mask before changing rendering, and never fill these gaps from DBZH unless explicitly implementing a clearly labelled derived product.
